/* This file contains many of the javaScript functions
 * used in the other php files. A majority of the functions
 * in this file are for the purpose of validation of user input.
 */
$(function() {
    $( document ).tooltip();
});
/* Datepicker is a javaScript widget that has the 
 * calendar dropdown used in the home page when users
 * select a departure and return date. This function also ensures that
 * the return date is not set to before the departure date.
 */
$(function() { 
...
/* OneWay() is called when a user checks that they would like to 
 * search for a one way flight. This function blocks out the return
 * date field when one way is checked and hides the calendar for 
 * return date on the home screen.
 */
function OneWay() {
...
/*showValue() displays the value selected
 * by the user on the price scale on the home page.
 */
...
/* Enables dropdown multiselect for airline options on home page.
 * If no airline is selected, it will default to "No Preference." 
 */
...
/*CodeArray holds the codes for all possible destinations */
...
/* The departure date and return date fields allow for a user to 
 * manually input a date or select a date from the calendar. If a 
 * user manually inputs a date in we need to make sure it is in the 
 * right format. This function parses the string and makes sure it matches
...
/*The form on the home page takes in a lot of user input so 
 * we use the validate function make sure all required fields
 * are filled out and filled correctly. Bootbox displays the 
 * error message in a nicely formatted way. 
 */
function validate(){
...
/* Google's QPX express API only allows a query of at most 9
 * passengers. So we need to check to make sure the user is not inputting 
 * more than 9 passengers total.
 */
...
/* This function increments the and decrements the passenger
 * count based on if the plus or minus button is pushed.
 */
...
/* Code to figure out how much time is remaining for the countdown clock.
 * It takes in the time the user inputs for the background search
 * and converts that to seconds and passes it to FlipClock which 
 * is a built in javascript item.
 */
